class Admin::NotificationsController < Admin::ApplicationController
  ...
  def mark_as_read
  	...
  	if @notification.resource_type == "company"  
  	  ...
  	  JobDescription.find(@notification.resource_id).update(update_button: true, edit_user_id: current_user.id)	if role_exists_for JobDescription.find(@notification.resource_id)
  ...
  def role_exists_for resource 
    if role = Role.find_by(role: "owner", user_id: current_user.id, resource_id: resource.id, resource_type: "job description")
      role
    end
  end
end
