class Admin::RolesController < Admin::ApplicationController
  ...
  def set_ownership_for_associated_resources
    if @resource.job_descriptions.any?	
      @resource.job_descriptions.each do |jd|
        if role_with_no_owner?(jd.id, "job description")
          role = Role.find_by role: "no owner", resource_id: jd.id, resource_type: "job description"
          role.update(role: "owner", user_id: current_user.id) if role
        else
          Role.create role: "owner", user_id: current_user.id, resource_id: jd.id, resource_type: "job description" unless Role.where(role: "owner", user_id: current_user.id, resource_id: jd.id, resource_type: "job description").any?
        end
      end
    end
    ...
    if @resource.applicants.any?
      @resource.applicants.each do |applicant|
        if role_with_no_owner?(applicant.id, "applicant")
  ...
  def unset_ownership_for_associated_resources
      ...
      if @resource.job_descriptions.any?	
        @resource.job_descriptions.each do |jd|
          role = Role.find_by role: "owner", user_id: current_user.id, resource_id: @resource.id, resource_type: "job description"
          role.update(role: "no owner", user_id: nil) if role
